  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/auth", {
        ...
        body: JSON.stringify({ action: mode, name, email, password, role }),
      });
  ...
  return (
    <main className="mx-auto grid w-full max-w-5xl flex-1 items-center gap-10 px-5 py-10 lg:grid-cols-[0.9fr_1.1fr]">
      <section className="max-w-xl">
        ...
        <h1 className="reel-title mt-4 text-5xl leading-[0.95] sm:text-6xl">Build the cast—or find the face.</h1>
        <p className="mt-5 max-w-lg text-base leading-7 text-grey">Creators develop original AI actors and stories. Brands discover production-ready talent for reels, ads, micro dramas, and UGC.</p>
      ...
      <section className="rounded-2xl border border-line bg-white/[0.045] p-5 shadow-2xl backdrop-blur-xl sm:p-7">
        ...
        <form onSubmit={submit} className="mt-6 space-y-5">
          {mode === "signup" && <fieldset>
            <legend className="mb-2 text-xs font-semibold uppercase tracking-[0.14em] text-grey">I am joining as</legend>
            <div className="grid gap-3 sm:grid-cols-2">
              {(["creator", "brand"] as SignupRole[]).map((option) => <label key={option} className={`cursor-pointer rounded-xl border p-4 transition-colors ${role === option ? "border-accent bg-accent/10" : "border-line hover:border-white/25"}`}>
                <input type="radio" name="role" value={option} checked={role === option} onChange={() => setRole(option)} className="sr-only" />
                <span className="block text-sm font-semibold">{option === "creator" ? "Creator" : "Brand"}</span>
                <span className="mt-1 block text-xs leading-5 text-grey">{option === "creator" ? "Create actors, scenes, and series." : "Cast actors for campaigns and content."}</span>
              </label>)}
            </div>
          </fieldset>}
          ...
          <button type="submit" disabled={busy} className="w-full rounded-full bg-accent px-5 py-3 text-sm font-semibold text-white hover:opacity-90 disabled:opacity-50">{busy ? "Please wait…" : mode === "signup" ? `Join as ${role}` : "Sign in"}</button>
        </form>
      </section>
    </main>
  );
